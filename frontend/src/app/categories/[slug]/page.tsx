...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return {
    title: `${category.name} | خرید بازی ${category.nameEn} برای PS5`,
    description: category.seoDescription || category.description || `خرید بهترین بازی‌های ${category.name} برای پلی‌استیشن ۵ با قیمت مناسب و تحویل فوری`,
...
export default async function CategoryPage({ params, searchParams }: Props) {
  ...
  const jsonLd = {
    ...
    name: category.name,
    description: category.seoDescription,
    url: `https://gameclub-iran.com/categories/${category.slug}`,
    breadcrumb: {
      '@type': 'BreadcrumbList',
      itemListElement: [
        {
          ...
          position: 1,
          name: 'خانه',
          item: 'https://gameclub-iran.com'
        },
        {
          '@type': 'ListItem',
          position: 2,
          name: 'دسته‌بندی‌ها',
          item: 'https://gameclub-iran.com/categories'
        },
        {
          '@type': 'ListItem',
          position: 3,
          name: category.name,
          item: `https://gameclub-iran.com/categories/${category.slug}`
        }
      ]
    },
    ...
    itemListElement: rawGames.map((game: any, index: number) => ({
      '@type': 'ListItem',
      position: index + 1,
      url: `https://gameclub-iran.com/games/${game.slug}`,
      name: game.title
    }))
  };
      ...
      <div className="container mx-auto px-4">
          ...
          <div className="flex flex-col items-center gap-6 text-center md:flex-row md:text-right">
            ...
            <div className="hidden md:block">
              <div className="rounded-2xl bg-slate-50 px-6 py-4 text-center">
                ...
                <div className="text-sm text-slate-500">بازی موجود</div>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="mb-8 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div className="text-sm text-slate-500">
            نمایش {mappedGames.length} بازی از {pagination?.total || 0}
          </div>
        ...
        {mappedGames.length > 0 ? (
          ...
          <div className="rounded-3xl border border-dashed border-slate-300 bg-white p-12 text-center">
            ...
            <h3 className="text-lg font-bold text-slate-900">بازی یافت نشد</h3>
            <p className="mt-2 text-slate-500">در حال حاضر هیچ بازی در این دسته‌بندی وجود ندارد.</p>
          </div>
        )}
